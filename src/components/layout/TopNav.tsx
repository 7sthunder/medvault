...
export function TopNav({ user }: TopNavProps) {
  ...
  const themeHeaderStyles = {
    ...
    plain: "bg-white/95 dark:bg-slate-900/95 border-b border-slate-200/80 dark:border-slate-800 shadow-xs text-slate-900 dark:text-slate-100 backdrop-blur-xl",
  }[theme] || "bg-slate-900/80 border-b border-border/70 text-slate-100 backdrop-blur-2xl shadow-xs";

  return (
    <header className={cn("sticky top-0 z-20 flex h-16 w-full items-center justify-between px-3 sm:px-6 transition-all duration-300", themeHeaderStyles)}>
      {/* Left section: mobile brand mark + breadcrumbs + portal badge */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <div className="md:hidden shrink-0">
          <Brand size={30} showWordmark={false} href={isCaregiver ? "/caregiver" : "/dashboard"} />
        </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        <NotificationBell />
        <div className="h-4 w-px bg-slate-300 dark:bg-white/20" aria-hidden="true" />
