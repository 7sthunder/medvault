...
export const PENDING_EVENT_STATUSES: readonly DoseEventStatus[] = ["upcoming", "due", "snoozed"] as const;
...
export function extendDeadlineForSnooze(missedDeadline: Date, snoozeUntil: Date, missedAfterMinutes: number): Date {
...
export function isPastDeadline(event: Pick<PendingEventLike, "scheduledFor" | "missedDeadline">, now: Date, missedAfterMinutes: number): boolean {
  const deadline = event.missedDeadline ?? computeOriginalDeadline(event.scheduledFor, missedAfterMinutes);
 ...
 */
export function deriveNextStatus(evt: PendingEventLike, now: Date, missedAfterMinutes: number): DerivedState {
  if (isPastDeadline(evt, now, missedAfterMinutes)) {
    const deadline = evt.missedDeadline ?? computeOriginalDeadline(evt.scheduledFor, missedAfterMinutes);
  ...
  if (evt.status === "snoozed" && evt.snoozeUntil !== null && now.getTime() >= evt.snoozeUntil.getTime()) {
...
export function canTakeDoseStatus(status: DoseEventStatus): boolean {
  return (
    status === "due" || status === "snoozed" || status === "upcoming" || status === "missed"
  );
}

/** §10.4 — an `upcoming` dose may only be taken inside the pre-dose reminder window. */
export function canTakeUpcoming(scheduledFor: Date, now: Date, reminderBeforeMinutes: number): boolean {
...
export function canSnoozeDoseStatus(status: DoseEventStatus, snoozeCount: number, maxSnoozes: number): boolean {
  return (
    (status === "due" || status === "snoozed" || status === "upcoming") &&
...
export function applySnooze(
  ...
  const baseDeadline = evt.missedDeadline ?? computeOriginalDeadline(evt.scheduledFor, opts.missedAfterMinutes);
  const missedDeadline = extendDeadlineForSnooze(baseDeadline, snoozeUntil, opts.missedAfterMinutes);
