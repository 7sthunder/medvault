...
describe("scheduleSchema", () => {
  ...
  it("accepts once-daily and twice-daily schedules", () => {
    expect(
      scheduleSchema.safeParse({ slots: [slot("08:00", [0, 1, 2, 3, 4, 5, 6])] }).success,
    ).toBe(true);
    expect(
      scheduleSchema.safeParse({
        slots: [slot("08:00", [0, 1, 2, 3, 4, 5, 6]), slot("20:00", [0, 1, 2, 3, 4, 5, 6])],
      }).success,
    ).toBe(true);
  });
  ...
  it("rejects bad times, empty daysets, duplicate days, and >6 slots", () => {
    expect(
      scheduleSchema.safeParse({ slots: [slot("24:00", [0, 1, 2, 3, 4, 5, 6])] }).success,
    ).toBe(false);
    ...
    const many = Array.from({ length: 7 }, (_, i) =>
      slot(`${String(i + 8).padStart(2, "0")}:00`, [1]),
    );
  ...
  it("allows the same time on disjoint daysets", () => {
    expect(
      scheduleSchema.safeParse({ slots: [slot("08:00", [1]), slot("08:00", [3])] }).success,
    ).toBe(true);
  });
  ...
  it("tolerates per-slot dosage override and instruction text", () => {
    expect(
      scheduleSchema.safeParse({
        slots: [slot("08:00", [1], { dosageAmount: "250", instructionOverride: "half tablet" })],
      }).success,
    ).toBe(true);
  });
});
...
describe("doseActionSchema", () => {
  ...
  it("accepts take/snooze and skip with an optional reason", () => {
    ...
    expect(doseActionSchema.safeParse({ doseId, action: "skip", skipReason: "" }).success).toBe(
      true,
    );
  });
  ...
  it("rejects unknown actions, non-uuid ids, and over-long reasons", () => {
    ...
    expect(
      doseActionSchema.safeParse({ doseId, action: "skip", skipReason: "x".repeat(201) }).success,
    ).toBe(false);
  });
});
...
describe("caregiverSchema", () => {
  it("invites with permissions and validates email/message bounds", () => {
    ...
    expect(caregiverInviteSchema.safeParse({ ...valid, message: "x".repeat(301) }).success).toBe(
      false,
    );
  });
...
describe("settingsSchema", () => {
  it("profile: trims, validates timezone and email", () => {
    ...
    expect(profileSchema.safeParse({ name: "x", email: "a@b.co", timezone: "UTC" }).success).toBe(
      false,
    );
  });
  ...
  it("reminders: honours ranges and pref shapes", () => {
    ...
    expect(reminderSettingsSchema.safeParse({ ...valid, missedAfterMinutes: 200 }).success).toBe(
      false,
    );
    expect(
      reminderSettingsSchema.safeParse({
        ...valid,
        caregiverAlertPrefs: { ...valid.caregiverAlertPrefs, adherenceDropThreshold: 150 },
      }).success,
    ).toBe(false);
  });
  ...
  it("appearance: theme/density are enum-restricted", () => {
    expect(
      appearanceSchema.safeParse({ theme: "dark", reduceMotion: true, uiDensity: "compact" })
        .success,
    ).toBe(true);
    expect(
      appearanceSchema.safeParse({ theme: "neon", reduceMotion: false, uiDensity: "compact" })
        .success,
    ).toBe(false);
  });
});
...
describe("reportsSchema", () => {
  it("accepts a valid range and rejects reversed/oversized ones", () => {
    expect(
      reportsSchema.safeParse({ granularity: "daily", from: "2026-05-01", to: "2026-05-10" })
        .success,
    ).toBe(true);
    expect(
      reportsSchema.safeParse({ granularity: "monthly", from: "2026-05-10", to: "2026-05-01" })
        .success,
    ).toBe(false);
    expect(
      reportsSchema.safeParse({ granularity: "daily", from: "2024-01-01", to: "2026-01-01" })
        .success,
    ).toBe(false);
  });
  ...
  it("enforces to ≤ today+1 against the server clock via the factory", () => {
    ...
    expect(
      server.safeParse({ granularity: "daily", from: "2026-05-01", to: "2026-05-21" }).success,
    ).toBe(true);
    expect(
      server.safeParse({ granularity: "daily", from: "2026-05-01", to: "2026-05-22" }).success,
    ).toBe(false);
  });
