...
async function seedEnabledMed(
  tx: DbTx,
  userId: string,
  startDate = "2026-01-05",
  slotsByTime = ["08:00", "20:00"],
): Promise<SeededMed> {
...
dbTests("doseEvents generation engine (§10.2)", () => {
  it("ensureDoseEvents writes 2/day through the horizon (boundary case)", async () => {
    await rollbackTx(async (tx, userId) => {
      const { medicationId } = await seedEnabledMed(tx, userId, "2026-01-05", ["08:00", "20:00"]);
      const res = await ensureDoseEvents(tx, {
        userId,
        medicationId,
        timeZone: "UTC",
        to: "2026-01-08",
  ...
  it("ensureDoseEvents is idempotent across the default horizon", async () => {
    await rollbackTx(async (tx, userId) => {
      ...
      const second = await ensureDoseEvents(tx, {
        userId,
        medicationId,
        timeZone: "UTC",
        from,
  ...
  it("voidFutureEvents cancels only unresolved future rows (history survives)", async () => {
    await rollbackTx(async (tx, userId) => {
      ...
      const earliest = before.sort(
        (a, b) => a.scheduledFor.getTime() - b.scheduledFor.getTime(),
      )[0]!;
      await tx
        .update(doseEvents)
        .set({ status: "taken", takenAt: new Date() })
        .where(eq(doseEvents.id, earliest.id));
  ...
  it("extendHorizon + catchUp are the same gap-filling pass", async () => {
    await rollbackTx(async (tx, userId) => {
      const idA = await seedEnabledMed(tx, userId, "2026-01-05", ["08:00"]);
      await ensureDoseEvents(tx, {
        userId,
        medicationId: idA.medicationId,
        timeZone: "UTC",
        to: "2026-01-06",
      });
  ...
  it("paused/archived meds are skipped (no generation)", async () => {
    await rollbackTx(async (tx, userId) => {
      const { medicationId } = await seedEnabledMed(tx, userId, "2026-01-05", ["08:00"]);
      await tx
        .update(medications)
