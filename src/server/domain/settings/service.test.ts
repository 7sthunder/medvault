...
import { DELETE_ACCOUNT_CONFIRM, DELETE_DATA_CONFIRM, deleteAccountSchema, deleteAllDataSchema } from "@/shared/validations/settings";
...
async function inRollbackTransaction<T>(fn: (tx: DbTx, actors: Actors) => Promise<T>): Promise<T> {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const actors: Actors = { owner: await make("Settings Owner"), stranger: await make("Settings Stranger") };
...
async function seedMedication(tx: DbTx, userId: string, overrides: Partial<typeof medications.$inferInsert> = {}) {
  const [row] = await tx
    .insert(medications)
    .values({
      id: uuidv7(),
...
async function countFor(tx: DbTx, table: typeof doseEvents | typeof doseActions | typeof notifications | typeof aiInsights | typeof adherenceDaily, userId: string) {
  const [row] = await tx
    .select({ n: count() })
...
dbTests("settings profile (§10.11)", () => {
  it(
    "reads the caller's own profile and never another account's",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
        ...
        const [strangerRow] = await tx.select({ name: users.name }).from(users).where(eq(users.id, stranger)).limit(1);
        expect(profile.name).not.toBe(strangerRow!.name);

        await expect(settingsService.getProfile(tx, "does-not-exist")).rejects.toBeInstanceOf(SettingsError);
      });
    },
    30_000,
  );
  ...
  it(
    "updates name/email/timezone and rejects an email that belongs to someone else",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
        const [strangerRow] = await tx.select({ email: users.email }).from(users).where(eq(users.id, stranger)).limit(1);
  ...
  it(
    "a timezone change invalidates the materialized day buckets (dose events survive)",
    async () => {
...
dbTests("settings reminders + appearance (§10.11)", () => {
  it(
    "creates a preferences row on first read and persists reminder defaults",
    async () => {
      await inRollbackTransaction(async (tx, { owner }) => {
        ...
        const saved = await settingsService.updateReminderSettings(tx, owner, {
          ...
          notificationPrefs: { doseReminders: false, caregiverMissedAlerts: true, insights: true, sounds: false },
  ...
  it(
    "bulk-toggles remindersEnabled for the caller's medications only",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
  ...
  it(
    "persists appearance and normalises a null adherence threshold",
    async () => {
      await inRollbackTransaction(async (tx, { owner }) => {
...
dbTests("settings data governance (§10.11)", () => {
  it(
    "data overview counts only the caller's rows and reports the real span",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
        const med = await seedMedication(tx, owner);
        const theirMed = await seedMedication(tx, stranger);
        await tx.insert(doseEvents).values([
          { id: uuidv7(), userId: owner, medicationId: med.id, scheduledFor: new Date("2026-02-01T08:00:00Z"), status: "taken" },
          { id: uuidv7(), userId: owner, medicationId: med.id, scheduledFor: new Date("2026-02-05T08:00:00Z"), status: "missed", isDemo: true },
          { id: uuidv7(), userId: stranger, medicationId: theirMed.id, scheduledFor: new Date("2026-02-09T08:00:00Z"), status: "taken" },
        ]);
  ...
  it(
    "exports CSV with deterministic columns and RFC-4180 quoting",
    async () => {
      await inRollbackTransaction(async (tx, { owner }) => {
        ...
        expect(meds.filename).toBe(`medvault-medications-${new Date().toISOString().slice(0, 10)}.csv`);
        const medHeader = meds.csv.split("\n")[0]!;
        expect(medHeader).toBe("id,name,dosage_amount,dosage_unit,status,start_date,end_date,reminders_enabled");
  ...
  it(
    "deleteAllData wipes clinical rows but keeps the account usable",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
        const med = await seedMedication(tx, owner);
        const theirMed = await seedMedication(tx, stranger);
        ...
        await tx.insert(doseActions).values({ id: uuidv7(), userId: owner, doseEventId: eventId, action: "take" });
        ...
        const [theirEvent] = await tx.select({ id: doseEvents.id }).from(doseEvents).where(eq(doseEvents.medicationId, theirMed.id));
  ...
  it(
    "deleteAccount removes the account itself and refuses the demo user",
    async () => {
      await inRollbackTransaction(async (tx, { owner }) => {
        const med = await seedMedication(tx, owner);
        await tx.insert(doseEvents).values({
  ...
  it(
    "caregiver alert rows are removed from both sides of a relationship",
    async () => {
      await inRollbackTransaction(async (tx, { owner, stranger }) => {
        const relId = uuidv7();
        ...
        const [alerts] = await tx
          ...
          .where(and(eq(caregiverAlerts.patientUserId, owner), eq(caregiverAlerts.caregiverUserId, stranger)));
        expect(Number(alerts!.n)).toBe(1);

        await settingsService.deleteAllDataRows(tx, owner);
        const [after] = await tx.select({ n: count() }).from(caregiverAlerts).where(eq(caregiverAlerts.patientUserId, owner));
...
describe("destructive-action typed confirmation (shared contract)", () => {
  it("requires the exact phrase, and rejects the other flow's phrase", () => {
    ...
    expect(deleteAllDataSchema.safeParse({ confirmation: DELETE_ACCOUNT_CONFIRM }).success).toBe(false);
    expect(deleteAllDataSchema.safeParse({ confirmation: "" }).success).toBe(false);

    expect(deleteAccountSchema.safeParse({ confirmation: DELETE_ACCOUNT_CONFIRM }).success).toBe(true);
    expect(deleteAccountSchema.safeParse({ confirmation: DELETE_DATA_CONFIRM }).success).toBe(false);
  });
});
