...
async function seedMedicationWithDoses(tx: DbTx, userId: string, days: number, status: "taken" | "missed") {
...
dbTests("caregiver invitation lifecycle (§10.6)", () => {
  it(
    "invite → preview → accept activates the pair with the invited permissions",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        ...
        await expect(caregiverService.accept(tx, caregiver, invite.token)).rejects.toBeInstanceOf(InviteError);
      });
    },
    30_000,
  );
  ...
  it(
    "blocks self-invites and duplicate pending invitations",
    async () => {
      await inRollbackTransaction(async (tx, { patient }) => {
        const [self] = await tx.select({ email: users.email }).from(users).where(eq(users.id, patient)).limit(1);
        ...
        const input = { email: "dupe@medvault.local", relationType: "family" as const, permissions: permissions() };
  ...
  it(
    "a revoked invitation can no longer be redeemed",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
...
dbTests("caregiver authorization matrix (§10.6 — strongest boundary)", () => {
  it(
    "patient-only management: a caregiver cannot edit or revoke someone else's relationship",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver, outsider }) => {
        ...
        await expect(
          caregiverService.updatePermissions(tx, outsider, rel.id, permissions({ viewAdherence: false })),
        ).rejects.toBeInstanceOf(AuthGateError);
        await expect(caregiverService.revoke(tx, outsider, rel.id)).rejects.toBeInstanceOf(AuthGateError);

        // The patient of the pair owns it and succeeds.
        const updated = await caregiverService.updatePermissions(tx, patient, rel.id, permissions({ viewMedications: true }));
  ...
  it(
    "cross-patient leakage denial: a caregiver cannot read an unrelated patient's data",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver, outsider }) => {
        const invite = await caregiverService.invite(tx, patient, {
  ...
  it(
    "revoking severs caregiver access immediately, including historical alerts",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
        ...
        const [alert] = await tx.select().from(caregiverAlerts).where(eq(caregiverAlerts.relationshipId, rel.id));
        ...
        await expect(caregiverService.alertAction(tx, caregiver, alert!.id, "acknowledge")).rejects.toBeInstanceOf(
          AuthGateError,
        );
        await expect(
          caregiverService.updatePermissions(tx, patient, rel.id, permissions({ viewAdherence: false })),
        ).rejects.toBeInstanceOf(AuthGateError);
  ...
  it(
    "alert detail is readable by the patient it is about, and by nobody else",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver, outsider }) => {
        const invite = await caregiverService.invite(tx, patient, {
  ...
  it(
    "permission gates: viewAdherence and canAcknowledgeAlerts are enforced server-side",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
        ...
        await caregiverService.accept(tx, caregiver, invite.token);
        await expect(caregiverService.patientOverview(tx, caregiver, patient, "UTC")).rejects.toBeInstanceOf(AuthGateError);

        await caregiverService.updatePermissions(tx, patient, (await activeRelationshipId(tx, patient)), permissions());
...
dbTests("caregiver alerts (§10.6)", () => {
  it(
    "dedupes missed-dose alerts once per (dose, relationship) and mirrors one notification",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
        ...
        await caregiverService.accept(tx, caregiver, invite.token);
        const { medicationId, eventIds } = await seedMedicationWithDoses(tx, patient, 1, "missed");
        const ref = { id: eventIds[0]!, medicationId, scheduledFor: new Date(), missedDeadline: new Date() };
  ...
  it(
    "no alert is raised when receiveMissedDoseAlerts is off",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
  ...
  it(
    "adherence-drop alert fires below the threshold, once per relationship per day",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
  ...
  it(
    "a healthy 7-day rate never raises an adherence-drop alert",
    async () => {
      await inRollbackTransaction(async (tx, { patient, caregiver }) => {
        const invite = await caregiverService.invite(tx, patient, {
