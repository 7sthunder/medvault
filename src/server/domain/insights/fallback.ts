...
function trailingAdherence(
  snapshot: InsightSnapshot,
  size: number,
): { recent: number | null; prior: number | null } {
  const withData = snapshot.daily
    .filter((d) => d.scheduled > 0 && d.adherencePercent !== null)
    .slice(-size * 2);
  ...
  const avg = (rows: typeof withData) =>
    rows.length === 0
      ? null
      : rows.reduce((acc, d) => acc + (d.adherencePercent ?? 0), 0) / rows.length;
  return { recent: avg(recent), prior: avg(prior) };
}

function medDifference(snapshot: InsightSnapshot): InsightItem | null {
  const meds = snapshot.medications.filter(
    (m) => m.taken + m.missed + m.skipped > 0 && m.adherencePercent !== null,
  );
...
function missedBucket(snapshot: InsightSnapshot): InsightItem | null {
  ...
  const label =
    worst.bucket === "morning"
      ? "morning"
      : worst.bucket === "afternoon"
        ? "afternoon"
        : worst.bucket === "evening"
          ? "evening"
          : "night";
  return {
  ...
  return {
    ...
    detail:
      streak > 0
        ...
        : `Log a few more days and ${BRAND.name} will spot deeper patterns for you.`,
    suggestedActionType: "encourage",
  };
}
