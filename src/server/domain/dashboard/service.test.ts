    ...
    await db.transaction(async (tx) => {
      const [user] = await tx
        ...
        .values({
          ...
          email: `dash-${uuidv7().slice(0, 8)}@meditrackai.local`,
          timezone: "UTC",
        })
...
dbTests("dashboardService.get (§11.4)", () => {
  it("returns an empty dashboard for a fresh user (no meds → no doses)", async () => {
  ...
  it("exposes a soonest unresolved dose as nextDose after seeding an active med", async () => {
    await rollbackTx(async (tx, userId) => {
  ...
  it("shows a missedToday count once time passes the missedAfterMinutes deadline", async () => {
    await rollbackTx(async (tx, userId) => {
...
dbTests("medicationExtras (adherence% + next dose)", () => {
  it("fills adherencePercent from resolved events and leaves null when no data", async () => {
    await rollbackTx(async (tx, userId) => {
      ...
      const [row] = await tx.select().from(doseEvents).where(eq(doseEvents.medicationId, idA));
      if (row) {
        await tx
          .update(doseEvents)
...
dbTests("dashboardService.latestInsight", () => {
  it("returns the newest insight row when one exists", async () => {
    await rollbackTx(async (tx, userId) => {
