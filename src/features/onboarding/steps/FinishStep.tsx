...
const ROWS: {
  key: keyof Pick<
    OnboardingInput,
    "missedAfterMinutes" | "snoozeMinutes" | "maxSnoozes" | "reminderBeforeMinutes"
  >;
  label: string;
  unit: string;
}[] = [
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start justify-between gap-4 rounded-lg border border-border-strong px-4 py-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            All schedules are generated in this local time.
          </p>
        </div>
        <span className="rounded-md bg-muted px-2 py-0.5 text-sm font-semibold text-ink-900">
      ...
      <dl className="grid gap-2 sm:grid-cols-2">
        {ROWS.map(({ key, label, unit }) => (
          <div
            key={key}
            className="flex items-center justify-between rounded-lg border border-border-strong px-4 py-3"
          >
      ...
      {values.addSampleMed && (
        <div className="flex items-start gap-3 rounded-lg border border-primary/40 bg-primary/5 px-4 py-3">
          ...
          <p className="text-sm text-ink-900">
            Metformin <span className="text-muted-foreground">500 mg</span> — twice a day at 08:00
            &amp; 20:00 — will be added to your vault.
          </p>
        </div>
      )}
    </div>
