      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16, width: "100%" }}>
            <motion.div animate={{ width: ["0%", "100%", "100%"] }} transition={{ duration: 4.5, repeat: Infinity, times: [0.1, 0.35, 1], ease: "easeOut" }} className="bg-background h-3 rounded-md" />
            <motion.div animate={{ width: ["0%", "85%", "85%"] }} transition={{ duration: 4.5, repeat: Infinity, times: [0.15, 0.4, 1], ease: "easeOut" }} className="bg-background h-3 rounded-md" />
            <motion.div animate={{ width: ["0%", "60%", "60%"] }} transition={{ duration: 4.5, repeat: Infinity, times: [0.2, 0.45, 1], ease: "easeOut" }} className="bg-background h-3 rounded-md" />
            <motion.div animate={{ width: ["0%", "75%", "75%"] }} transition={{ duration: 4.5, repeat: Infinity, times: [0.25, 0.5, 1], ease: "easeOut" }} className="bg-background h-3 rounded-md" />
          </div>
        </motion.div>
      </motion.div>
    </div>
...
function FeatureSmartReminders() {
  ...
  return (
    <div style={{ position: "relative", width: 340, height: 400, display: "flex", justifyContent: "center", alignItems: "center", margin: "0 auto" }}>
      ...
      >
        <div className="bg-bg-soft absolute top-0 left-1/2 z-10 h-5 w-20 -translate-x-1/2 rounded-b-lg" style={{ borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }} />
        <motion.div
          animate={{ y: [0, -400, -400] }}
        ...
        >
          {scrollItems.map((type, i) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div className="bg-border-strong mb-[6px] h-[10px] rounded" style={{ width: type % 2 === 0 ? "50%" : "70%" }} />
        ...
        >
          ...
          <div>
            <div className="text-ink-900 text-sm font-bold" style={{ marginBottom: 3, lineHeight: 1.2 }}>Take sugar tablets</div>
            <div className="text-ink-500 text-xs" style={{ lineHeight: 1.2 }}>Take 1 tablet after food</div>
          </div>
        </motion.div>
      </motion.div>
    </div>
    ...
    <div style={{ position: "relative", width: 340, height: 400, display: "flex", justifyContent: "center", alignItems: "center", margin: "0 auto" }}>
      ...
      >
          ...
          >
            {pin.main ? (
              <motion.div
                animate={{
                  scale: [1, 1, 1.25, 1.25],
                  boxShadow: ["0 0 0px rgba(6, 182, 212, 0)", "0 0 0px rgba(6, 182, 212, 0)", "0 16px 32px rgba(6, 182, 212, 0.6)", "0 16px 32px rgba(6, 182, 212, 0.6)"],
                }}
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div className="text-ink-900 text-sm font-bold" style={{ marginBottom: 2 }}>City Hospital</div>
...
const FEATURES = [
  ...
  },
  {
    ...
    body: "Set up your full medication schedule once. MedVault adapts to your routine and sends reminders at the exact right moment.",
    bullets: [
  ...
  },
  {
    ...
    body: "Locate the nearest verified hospitals, clinics, and pharmacies. Real-time hours, ratings, and turn-by-turn directions — all inside MedVault.",
    bullets: [
...
export default function Features() {
  ...
  return (
    <section id="features" className="overflow-hidden bg-white" style={{ padding: "100px 6% 60px" }}>
      <div ref={h2Ref} style={{ textAlign: "center", marginBottom: 100, opacity: h2In ? 1 : 0, transform: h2In ? "translateY(0)" : "translateY(30px)", transition: "all 0.8s ease" }}>
        ...
        {FEATURES.map((f, idx) => (
          ...
          >
            <div style={{ flex: 1, minWidth: 320, padding: "20px 0" }}>
              <div className={`text-[13px] font-extrabold tracking-[0.12em] uppercase ${f.kickerClass}`} style={{ marginBottom: 14 }}>
                {f.kicker}
              </div>
              <h3 className="font-heading text-ink-900" style={{ fontSize: "clamp(28px,3.5vw,44px)", fontWeight: 800, marginBottom: 20, letterSpacing: "-0.025em", lineHeight: 1.15 }}>
                {f.title}
              </h3>
              <p className="text-ink-600 text-[17px]" style={{ lineHeight: 1.8, marginBottom: 32, maxWidth: 640 }}>
                {f.body}
              </p>
              <ul className="grid gap-4" style={{ listStyle: "none", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
                {f.bullets.map((b, j) => (
                  <li key={j} className="text-ink-700 text-[15.5px] font-semibold" style={{ display: "flex", alignItems: "flex-start", gap: 12, lineHeight: 1.5 }}>
                    <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-extrabold ${f.chip}`} style={{ marginTop: 2 }}>
                      ✓
                    </span>
                    {b}
                  </li>
            ...
            <div style={{ flex: "1 1 45%", minWidth: 360, display: "flex", justifyContent: "center" }}>{f.mock}</div>
          </motion.div>
        ))}
      </div>
    </section>
